...
*/
   ...
   InitPll(0xA);	//设置PLL,使得SYSCLKOUT=OSCCLK*5,是不是此时的SYSCLKOUT就是150MHZ?

   // Initialize the peripheral clocks
   InitPeripheralClocks();	//设置高低速时钟并使能外设时钟
}
...
void InitPll(Uint16 val)
{
   ...
   // accordingly. 
      for(iVol= 0; iVol< ( (131072/2)/12 ); iVol++)		//等待PLL稳定,SYSCLKOUT从暂时的OSCCLK/2稳定到OSCCLK*5
      {
...
// HISPCP/LOSPCP prescale register settings, normally it will be set to default values
   SysCtrlRegs.HISPCP.all = 0x0001;		//默认的高速时钟为SYSCLKOUT/2
   SysCtrlRegs.LOSPCP.all = 0x0002;		//默认的低速时钟为SYSCLKOUT/4
